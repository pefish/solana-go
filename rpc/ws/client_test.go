...
func Test_AccountSubscribe(t *testing.T) {
	...
	c := Connect(context.Background(), "ws://api.mainnet-beta.solana.com:80")
	defer c.Close()
...
func Test_AccountSubscribeWithHttpHeader(t *testing.T) {
	...
	c := ConnectWithOptions(context.TODO(), "ws://api.mainnet-beta.solana.com:80", opt)
	defer c.Close()
...
func Test_ProgramSubscribe(t *testing.T) {
	...
	fmt.Println("Dialing")
	c := Connect(context.Background(), "wss://solana-api.projectserum.com")
	fmt.Println("Hello?")
	defer c.Close()
